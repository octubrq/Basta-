...
export default function FinalResultsPage() {
  const { user } = useAuth();
  const { finalResults, scrambleResults, sessionType, resetToLobby } = useGame();
  const standings = sessionType === 'scramble' ? (scrambleResults?.standings || []) : (finalResults?.standings || []);
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center pb-28 bg-gradient-to-b from-yellow-50 via-white to-pink-50">
      <div className="w-full text-center pt-8 pb-4">
        <p className="text-purple-300 font-body text-sm">{sessionType === 'scramble' ? '🔤 Letras Locas' : '🎯 ¡Basta!'} — Fin</p>
        <h1 className="font-display text-5xl font-bold mt-2 animate-bounce-in" style={{ color: isWinner ? '#F59E0B' : '#EC4899', textShadow: '2px 2px 0 rgba(0,0,0,0.05)' }}>
          {isWinner ? '🎉 ¡Victoria!' : '🏁 ¡Fin!'}
        </h1>
        {myPos > 0 && <p className="text-purple-400 font-body mt-2">Posición #{myPos}</p>}
      </div>
      ...
      <div className="w-full max-w-lg px-4 space-y-4">
        ...
        <div className="bg-white rounded-3xl p-4 shadow-md border-2 border-purple-100">
          <h3 className="font-display text-purple-500 font-bold mb-3">🏆 Ranking Final</h3>
          {standings.map((s, i) => (
        ...
        {sessionType === 'scramble' && scrambleResults?.log?.length > 0 && (
          <div className="bg-white rounded-3xl p-4 shadow-md border-2 border-green-200">
            <h3 className="font-display text-green-500 font-bold mb-3">📜 Palabras</h3>
            {scrambleResults.log.map((l, i) => (
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white to-transparent">
        <div className="max-w-lg mx-auto">
          <button onClick={resetToLobby} className="btn-purple w-full text-lg py-4">🏠 Volver al Lobby</button>
        </div>
      </div>
    </div>
  );
